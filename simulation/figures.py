...
def plot_price_lever(results: dict, path: str) -> None:
    ...
    ax.set_xlabel("petrostate export price $p_P$ ($/mcf)")
    ax.set_ylabel("takeoff threshold $x^\\dagger$")
    ax.set_title("The price lever moves the importer's threshold")
...
def plot_harvest_extend(results: dict, path: str) -> None:
    ...
    ax1.set_xlabel("constant price $p_P$ ($/mcf)")
    ax1.set_ylabel("present value of rents ($bn)")
    ax1.set_title("Harvest vs extend")
    ...
    if hv["discount_rate_crossover"] is not None:
        ax2.axvline(hv["discount_rate_crossover"], ls=":", color="#888",
                    label=f"crossover $r$ = {hv['discount_rate_crossover']:.3f}")
        ax2.legend(fontsize=8)
    ax2.set_xlabel("discount rate $r$")
    ax2.set_ylabel("rent-maximizing price $p_P^*$")
    ax2.set_title("Patience favors extending")
    fig.tight_layout(); fig.savefig(path, dpi=150); plt.close(fig)
...
def plot_magnitude(results: dict, path: str) -> None:
    ...
    ax.set_yscale("log")
    ax.set_ylabel("stake ($bn, log scale)")
    ax.set_title("The displacement is small; the capitalized stake is not")
    for i, v in enumerate(vals):
        ax.text(i, v * 1.1, f"${v:.0f}bn", ha="center", fontsize=8)
    fig.tight_layout(); fig.savefig(path, dpi=150); plt.close(fig)
